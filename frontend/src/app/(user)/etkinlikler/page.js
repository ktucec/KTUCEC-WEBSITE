...
export default function EventsPage() {
    ...
    const eventsData = [
        ...
        },
        {
            ...
            date: "25.03.2026",
            tag: "Arşiv",
            title: "Güz Dönemi Tanışma Toplantısı",
    ...
    return (
        <main className="pt-40 pb-7 overflow-x-hidden px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto relative z-10">
            ...
            <section className="mb-32 text-center relative z-10">
                <nav className="flex items-center justify-center gap-2 text-on-surface-variant/60 font-label-md text-label-md mb-6 uppercase tracking-widest">
                    <Link href="/" className="hover:text-primary transition-colors">Ana Sayfa</Link>
                    <span className="material-symbols-outlined text-[14px]">chevron_right</span>
                    <span className="text-primary font-bold">Etkinlikler</span>
                </nav>
            ...
            <div ref={containerRef} className="relative overflow-x-hidden w-full min-h-[1500px]">
                ...
                {events.map((event, index) => (
                    <div
                        key={event.id}
                        className={`relative flex flex-col md:flex-row items-center justify-between mb-64 group ${event.align === 'right' ? 'md:flex-row-reverse' : ''} ${event.status === 'past' ? 'opacity-60 hover:opacity-100 transition-opacity' : ''}`}
